# 사용법
# python LibriSpeech_prep.py --input_path ./raw_dataset/LibriSpeech/train-clean-100 --output_path ./raw_dataset/LibriSpeech/output
...
def process_all_speakers(input_folder, output_folder):
    for speaker in os.listdir(input_folder):
        speaker_path = os.path.join(input_folder, speaker)
        if os.path.isdir(speaker_path):
            combined_audio = AudioSegment.empty()
            for chapter in os.listdir(speaker_path):
                chapter_path = os.path.join(speaker_path, chapter)
                if os.path.isdir(chapter_path):
                    combined_audio += process_speaker_audios(chapter_path, speaker, output_folder)
            ...
            output_path = os.path.join(output_folder, f"{speaker}_normalized.wav")
            combined_audio.export(output_path, format="wav")
            print(f"{speaker}의 정규화된 파일이 생성되었습니다: {output_path}")
